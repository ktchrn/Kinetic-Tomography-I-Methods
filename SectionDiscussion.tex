...
%do I have to fit rotation curves & RGC & solar motions to subsets of Reid. is this what it's come to
...
%thing I could do: figure out the sigma_R**2 + sigma_Phi**2 at which my uncertainties and theirs are the same
...
%NB (kind of supported by Figure \ref{fig:maser_pie}).

%paragraph 5
Second, the optimistic option: \citet{Reid} make measurements that aren't wildly biased because the HMSFRs are preferentially located in a specific dynamical state. 
...
% this option is very poorly explained; what preferred dynamical state could a bunch of HMSFRs with very wacky peculiar velocties be in?
%they're all going in different directions, so it's option 1 and the answer is that the magnitude of the small-scale velocity dispersion is large enough that the effective SNR per patch is small?

%what is Reid claiming, exactly?
% mean motion, ~3 km/sec in each direction
% total standard deviation of about 10 km/sec (v obs uncertainty + virial term) per HMSFR
% outliers sigma-clipped out
% this is enough to not be affected by \emph{systematic} trends in HMSFR motions

%What are we claiming? 
% We agree on point-by-point kinematics; we have lots more data; we are systematics dominated; therefore, Reid isn't right?
