...
The matter density at a $\glon$, $\glat$, $d$, and $v$ in PPDV space is 
\begin{equation}
  \rho(\glon, \glat, d, v) = \frac{\rho(\glon, \glat, d)}{\sqrt{2 \pi \sigma_v(\glon, \glat, d)^2}} 
    \exp\left(- \frac{\left( v - \vlos(\glon, \glat, d) \right)^1}{2 \sigma_v(\glon, \glat, d)^2} \right),
\end{equation}
and the matter density at a $\glon$, $\glat$, and $v$ in PPV space is
\begin{equation}
  \rho(\glon, \glat, v) = \int_0^{d_{max}} \rho(\glon, \glat, d, v) \, \rm{d}d.
\end{equation}
(we apologize for the $\rm{d}d$; this is the only place where it shows up)
